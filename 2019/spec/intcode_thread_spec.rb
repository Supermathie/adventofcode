...
describe IntcodeThread do
  ...
  it "can use relative addresses" do
    prog = [109,1,204,-1,99]
    comp = IntcodeThread.new(prog).execute
    comp.join
    expect(comp.output.pop).to eq(109)
  end

  it "can use relative addresses (ex. 9a1)" do
    prog = [109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99]
    comp = IntcodeThread.new(prog).execute
    comp.join
    q = comp.output
    expect(q.length.times.map { q.pop }).to eq(prog)
  end

  it "can use relative addresses (ex. 9a2)" do
    prog = [1102,34915192,34915192,7,4,7,99,0]
    comp = IntcodeThread.new(prog).execute
    comp.join
    expect(comp.output.pop).to eq(34915192*34915192)
  end

  it "can use relative addresses (ex. 9a3)" do
    prog = [104,1125899906842624,99]
    comp = IntcodeThread.new(prog).execute
    comp.join
    q = comp.output
    expect(comp.output.pop).to eq(1125899906842624)
  end
end
